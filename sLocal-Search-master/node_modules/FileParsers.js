...
function ClassParser(p){	
	...
	for (var i = 0; i < data.numberOfGroups; i++){
        ...
        data.groups[group.time + "," + group.name] = group;
        
        if (typeof data.groupsByTime[group.time] === "undefined")
            data.groupsByTime[group.time] = [];
        
        data.groupsByTime[group.time].push(group);
	}
...
function StudentParser(p){
	...
	data.students = {};
	var index = 0;
	console.log( "data.numStudent: ", data.numStudent);
